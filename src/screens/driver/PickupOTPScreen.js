...
export default function DriverPickupOTPScreen({ route, navigation }) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      navigation.replace('PhotoUpload', { booking, stage: 'pickup' });
    } catch (err) {
      if (err.code === 'WRONG_OTP' || err.status === 400) {
        shake();
        setDigits(['', '', '', '']);
        inputRefs[0].current?.focus();
        Alert.alert('Incorrect code', 'Ask the sender to read it again.');
      } else {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <Text style={styles.subtitle}>Ask the sender to read you their 4-digit code</Text>
      ...
      >
        {digits.map((d, i) => (
          <TextInput
            ...
            maxLength={1}
            textAlign="center"
            selectionColor={theme.colors.volt}
            caretHidden
          />
        ))}
      </Animated.View>
